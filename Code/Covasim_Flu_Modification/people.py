...
class People(cvb.BasePeople):
    ...
    **Examples**::
    ...
    def infect(self, inds, hosp_max=None, icu_max=None, source=None, layer=None, variant=0):
        ...
        # LP_mod: Below is the default setting for recording transmissionsde, but currently not used
        # for i, target in enumerate(inds):
        #     # entry = dict(source=source[i] if source is not None else None, target=target, date=self.t, layer=layer, variant=variant_label)   ## default
        ...
        is_symp = cvu.binomial_arr(symp_probs) # Determine if they develop symptoms  ## return False/True list
        ...
        # LP_MOD: Modified transmission logging with symptomatic information
        # LP_MOD: Extended infection log with age information and symptomatic onset dates
        for i, target in enumerate(inds):
            entry = dict(source=source[i] if source is not None else None, target=target, date=self.t, layer=layer,variant=variant_label,
                         age_source=self.age[source[i]] if source is not None else None, age_target=self.age[target],
                         date_symptomatic = self.date_symptomatic[target])
            self.infection_log.append(entry)
        ...
        if self.pars['use_waning']:
            ...
            # print ("-----LP_people.py:asymp_inds:",asymp_inds)   ## LP_note: asymp_inds是list[asymp个体的uid]
