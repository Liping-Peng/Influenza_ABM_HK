...
def main_calibrate(calib_interventions):
    """
    Main function to run the model calibration process.

    Args:
        calib_interventions (list): List of interventions to use during calibration

    Returns:
        None: Runs calibration and saves results to files
    """

    # Create simulation object for calibration
    ...
    # Define calibration parameter ranges
    # Format: {parameter_name: [best_guess, lower_bound, upper_bound]}
    calib_pars = dict(
        beta=[set_beta, set_beta-0.0015, set_beta+0.003],
        pop_infected=[set_infected, set_infected-2000, set_infected+10000],
        LP_sus_0_25  = [1, 0.5, 2.0],
    ...
    # Check if calibration database exists and remove it if present
...
def main_simulation(begin_seed, N):
    """
    Main function to run simulation scenarios with calibrated parameters.
    Main function to run simulation scenarios with calibrated parameters.

    Args:
        begin_seed (int): Starting random seed for simulation ensemble
        N (int): Number of simulations to run

    Returns:
        None: Runs simulations and saves results to files
    """

    """ (1) Run single scenario and output detailed agent-level results """
    for n in range(begin_seed, begin_seed + N):
        # Update parameters with optimal calibrated values
        ...
        # ## option 1:  Create new population for each simulation
        ...
        ## option 2: Create population once and load for subsequent simulations
        if n == 0:
        ...
        # Load pre-created population for all simulations
        with sc.timer('loading'):
            sim_base = cv.Sim(pars_base, datafile=df_base, label=scenario,
                              interventions=scen_intervention,
...
if __name__ == "__main__":
    # Set working directory - same in base_func.py
    ...
    list_Scenario = [
        ...
        # 'S11_vg0_vc0_tip10_tie0_tida0_mc0_me0_intvd0_tidd0_tida1'
    ]
    ...
    for scenario in list_Scenario:
        ...
        pars_base = dict(
            ...
            beta_dist = dict(dist='uniform', par1=1.0, par2=1.0),   # Distribution to draw individual level transmissibility, assume no difference in individual level transmissibility
            viral_dist = dict(frac_time=0.4, load_ratio=2, high_cap=4),  # Time varying viral load (transmissibility)
            asymp_factor = 0.5,   # Relative transmissibility of asymptomatic cases

            # Immunity
            nab_init = dict(dist='normal', par1=0,par2=2),  # Return Normal(μ=0, σ=2)  # Parameters for the distribution of the initial level of log2(nab) following natural infection, taken from fig1b of https://doi.org/10.1101/2021.03.09.21252641
            nab_decay = dict(form='nab_growth_decay', growth_time=14, decay_rate1=0.14, decay_time1=365, decay_rate2=np.log(2) / 60, decay_time2=720),  # Define immunity protection waning pattern
            nab_boost = 4,  # Boosting existing immunity (four-fold rise in HAI titer)
            trans_redux = 1,  # Reduction in transmission for breakthrough infections, assume 1 (no reference)
