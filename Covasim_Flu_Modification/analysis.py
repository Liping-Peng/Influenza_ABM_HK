...
class Fit(Analyzer):
    ...
    **Example**::
    ...
    def __init__(self, sim, weights=None, keys=None, custom=None, compute=True, verbose=False, die=True, label=None, **kwargs):
        ...
        self.weights    = sc.mergedicts({'cum_deaths':10,
                                         'cum_diagnoses':5,
                                         'new_infections.0.24' : 0.24,   # LP_mod: Added age-stratified infection targets (0–24, 25–44, 45–64, 65+) with weights proportional to their population shares.
                                         'new_infections.25.44': 0.31,
                                         'new_infections.45.64': 0.30,
                                         'new_infections...65' : 0.15,
                                         'new_absent':           0.10},  # LP_mod: Added school absenteeism as a calibration target with relative weight 0.10.
                                        weights)
        self.keys       = keys
        ...
        if compute:
            # self.compute()
            self.compute(sim=sim)  # LP_mod: Modified to pass the `sim` into compute().
    ...
    def compute(self,sim):
        ...
        # self.compute_gofs()
        self.compute_gofs(sim=sim)   # LP_mod: Modified to pass the `sim` into compute_gofs().
        self.compute_losses()
    ...
    # LP_mod: Added sim as an explicit argument, enabling integration of additional calibration data such as age-specific infections.
    def compute_gofs(self, sim, **kwargs):
        ...
        for key in self.pair.keys():
            ...
            # self.gofs[key] = cvm.compute_gof(actual, predicted, **kwargs)  ### default use: normalized absolute error
            self.gofs[key] = cvm.compute_gof(actual, predicted, normalize=True, use_squared=True, as_scalar = 'mean')  # LP_mod: use mean squared error


        # LP_mod: Enables calibration against age-specific empirical incidence data and school absenteeism records.
        if ("calibration" in sim.label) or ("S0" in sim.label ):
            # LP_mod: 1. Age-specific infection metrics calculation
            if sim.pars['LP_mismatch_age']:   # LP_mod: Control flag for age-specific infection calibration
                # LP_mod: Process infection log to calculate daily new infections by age group
                inflog = sim.people.infection_log
                df_inflog = pd.DataFrame(inflog)
                bins = [0, 25, 45, 65, 100]
                labels = ['0-24', '25-44', '45-64', '>=65']
                df_inflog['ag_target'] = pd.cut(df_inflog['age_target'], bins=bins, labels=labels,include_lowest=True)
                df_inflog = df_inflog.pivot_table(index='date', columns='ag_target', values='source', aggfunc='count',fill_value=0).reset_index(drop=False)
                date_range = pd.DataFrame({'date': range(sim.t+1)})  # LP_mod: Create full date range to include days with zero infections
                df_inflog = pd.merge(df_inflog, date_range, on='date', how='right').fillna(0)
                for t in df_inflog['date']:  # LP_mod: Apply time-dependent rescaling factors
                    df_inflog.loc[df_inflog['date'] == t, ['0-24', '25-44', '45-64', '>=65']] = df_inflog.loc[df_inflog['date'] == t, ['0-24', '25-44', '45-64', '>=65']] * sim.rescale_vec[t]
                group_infection = df_inflog[['0-24', '25-44', '45-64', '>=65']]
                group_infection.columns = ["new_infections.0.24", "new_infections.25.44", "new_infections.45.64", "new_infections...65"]

                # LP_mod: Calculate GOF metrics for each age group
                i = 0
                for group, i in zip(["new_infections.0.24", "new_infections.25.44", "new_infections.45.64", "new_infections...65"], range(4)):
                    actual = sim.datafile[group].tolist()
                    predicted = group_infection[group].tolist()
                    self.gofs[group] = cvm.compute_gof(actual, predicted, normalize=True, use_squared=True,as_scalar='mean')


            # LP_mod: 2. School absenteeism metrics calculation
            if sim.pars['LP_mismatch_absent']:  # LP_mod: Control flag for school absenteeism calibration
                # LP_mod: Use isolation log to calculate daily school absenteeism
                isolog = sim.people.LP_iso_log
                df_isolog = pd.DataFrame(isolog)
                df_isolog_sc = df_isolog[(df_isolog['age'] >= 6) & (df_isolog['age'] <= 18)]
                time_range = np.arange(sim.t + 1)
                is_iso = (df_isolog_sc['date_iso'].values[:, None] <= time_range) & (df_isolog_sc['end_iso'].values[:,None] >= time_range)
                iso_counts = is_iso.sum(axis=0)
                predicted = iso_counts * sim.rescale_vec

                # LP_mod: Load actual absenteeism data
                actual = sim.datafile["new_absent"].tolist()
                # LP_mod: Handle season-specific data by truncating early data
                if sim.pars['LP_season'] == 6:
                    actual = actual[90:]
                    predicted = predicted[90:]
                elif sim.pars['LP_season'] == 4:
                    actual = actual[70:]
                    predicted = predicted[70:]
                elif sim.pars['LP_season'] == 5:
                    actual = actual[75:]
                    predicted = predicted[75:]

                # LP_mod: Filter out NaN values from actual data
                actual = np.array(actual)
                valid_indices = ~np.isnan(actual)
                actual    = actual[valid_indices]
                predicted = predicted[valid_indices]

                self.gofs["new_absent"] = cvm.compute_gof(actual, predicted, normalize=True, use_squared=True, as_scalar='mean')
    ...
    def compute_mismatch(self, use_median=False):
        ...
        # print ('********LP_analysis.py: self.mismatches*****',self.mismatches)
...
class Calibration(Analyzer):
    ...
    **Example**::
    ...
    def worker(self):
        ...
        output = study.optimize(self.run_trial, n_trials=self.run_args.n_trials)   ## LP_note: self.run_trial is the objective function for study.optimize
        return output
    ...
    # LP_mod: Added function to visualize age-specific infection calibration results
    def LP_plot_age(self, is_save=False, save_path= None, sim_type='after calib'):
        '''
        LP_mod: Plot age-specific infection data vs simulation results for calibration validation
        Parameters:
            is_save: Whether to save the plot
            save_path: Path to save the plot if is_save is True
            sim_type: Type of simulation to use ('after calib' for calibrated, 'save_sim' for saved simulation)
        '''
        # LP_mod: Select appropriate simulation object based on type
        if sim_type == 'after calib':
            sim_after = self.after
        elif sim_type == 'save_sim':
            sim_after = self

        # LP_mod: Extract age-specific infection data from calibration dataset
        data = sim_after.datafile
        data = data[(data['date']>=sim_after.pars['start_day']) & (data['date']<=sim_after.pars['end_day'])]
        data = data[["new_infections.0.24",    "new_infections.25.44",   "new_infections.45.64",  "new_infections...65"]]

        # LP_mod: Method 1: Calculate age-specific infections from people states
        list_edges = np.array([0, 25, 45, 65, 100])
        list_bins = list_edges[:-1]  # Don't include the last edge in the bins
        state = 'exposed'   # list_states = ['exposed', 'severe', 'dead', 'tested', 'diagnosed']
        dict1 = {}
        for t in range(sim_after.t+1):
            inds = sc.findinds(sim_after.people[f'date_{state}'], t)
            b, _ = np.histogram(sim_after.people.age[inds], list_edges)
            dict1[t] = b * sim_after.pars['pop_scale']
        df_sim = pd.DataFrame(dict1).T
        # print (df_sim)

        # LP_mod: Method 2: Extract age-specific infections from infection log (preferred method)
        inflog = sim_after.people.infection_log
        df_inflog = pd.DataFrame(inflog)
        bins = [0, 25, 45, 65, 100]
        labels = ['0-24', '25-44', '45-64', '>=65']
        df_inflog['ag_target'] = pd.cut(df_inflog['age_target'], bins=bins, labels=labels, include_lowest=True)
        df_inflog = df_inflog.pivot_table(index='date', columns='ag_target', values='source', aggfunc='count',fill_value=0).reset_index(drop=False)
        date_range = pd.DataFrame({'date': range(sim_after.t+1)})
        df_inflog = pd.merge(df_inflog, date_range, on='date', how='right').fillna(0)
        for t in df_inflog['date']:
            df_inflog.loc[df_inflog['date'] == t, ['0-24', '25-44', '45-64', '>=65']] = df_inflog.loc[df_inflog['date'] == t, ['0-24', '25-44','45-64', '>=65']] * sim_after.rescale_vec[t]
        df_inflog.drop('date', axis=1, inplace=True)
        # print (df_inflog)


        # LP_mod: Create comparison plot for each age group
        fig = pl.figure(figsize=(5, 8), dpi=200)
        label_group = {0: '0-24', 1: '25-44', 2: '45-64', 3: '>=65'}
        mpl.rcParams.update({'font.size': 5})
        for i in range(0, 4):
            ax = pl.subplot2grid((4, 1), (i, 0))
            ax.plot(range(len(data)),   data.iloc[:, i], color='black', label='Data')
            ax.plot(range(len(df_sim)), df_sim.iloc[:, i], color='red', label='After (people)')
            ax.plot(range(len(df_inflog)), df_inflog.iloc[:, i], color='blue', label='After (inflog)')  # (preferred method)
            ax.set_title(f'Age group {label_group[i]}')
        ax.legend()
        pl.subplots_adjust(hspace=0.37)
        if is_save:
            pl.savefig(save_path, bbox_inches='tight')
        pl.show()


    # LP_mod: Added function to visualize school absenteeism calibration results
    def LP_plot_absent(self, is_save=False, save_path= None, sim_type='after calib'):
        '''
        LP_mod: Plot school absenteeism data vs simulation results for calibration validation
        Parameters:
            is_save: Whether to save the plot
            save_path: Path to save the plot if is_save is True
            sim_type: Type of simulation to use ('after calib' for calibrated, 'save_sim' for saved simulation)
        ...
        # LP_mod: Extract school absenteeism data from calibration dataset
        data = sim_after.datafile
        data = data[["new_absent"]]


        # LP_mod: Calculate absenteeism from isolation log (preferred method - consistent with simulation output)
        isolog = sim_after.people.LP_iso_log
        df_isolog = pd.DataFrame(isolog)
        df_isolog_sc = df_isolog[(df_isolog['age'] >= 6) & (df_isolog['age'] <= 18)]
        time_range = np.arange(sim_after.t + 1)
        is_iso = (df_isolog_sc['date_iso'].values[:, None] <= time_range) & (df_isolog_sc['end_iso'].values[:,None] >= time_range)
        iso_counts = is_iso.sum(axis=0)
        predicted_rescale = iso_counts * sim_after.rescale_vec

        # LP_mod: Handle season-specific data by truncating early problematic data
        if sim_after.pars['LP_season'] == 6:
            actual = data["new_absent"].tolist()[90:]
            predicted_rescale = predicted_rescale[90:]
        elif sim_after.pars['LP_season'] == 4:
            actual = data["new_absent"].tolist()[70:]
            predicted_rescale = predicted_rescale[70:]
        elif sim_after.pars['LP_season'] == 5:
            actual = data["new_absent"].tolist()[75:]
            predicted_rescale = predicted_rescale[75:]
        else:
            actual = data["new_absent"].tolist()
        # print(predicted_rescale)

        # LP_mod: Create comparison plot for school absenteeism
        fig = pl.figure(figsize=(4, 3), dpi=200)
        mpl.rcParams.update({'font.size': 5})
        pl.plot(range(len(actual)),   actual, color='black', label='Data')
        pl.plot(range(len(predicted_rescale)), predicted_rescale, color='red', label='After(isolog)')
        pl.title(f'Absenteesim')
        pl.legend()
        if is_save:
            pl.savefig(save_path, bbox_inches='tight')
        pl.show()
    ...
    **Example**::
    ...
    def r0(self, recovered_only=False):
        ...
        try:
            ...
            ## LP_note: only include first 30 days' results (must set: recovered_only = True)
            for i, node in self.graph.nodes.items():
                if i is None or np.isnan(node['date_exposed']) or (recovered_only and node['date_recovered']>20):
                    continue
                n_infected_20d.append(self.graph.out_degree(i))
        ...
        # print("[r0]:", np.mean(n_infected))
        # print ("[r0 first 20 days]:", np.mean(n_infected_20d))
