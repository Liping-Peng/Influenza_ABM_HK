...
class Sim(cvb.BaseSim):
    ...
    **Examples**::
    ...
    def init_people(self, popdict=None, init_infections=False, reset=False, verbose=None, **kwargs):
        ...
        LP_mod:
        - Enhanced with seasonal influenza modeling parameters and extended data storage capabilities for immunological imprinting and age-stratified transmission dynamics.
        - User-provided parameter values override default values for customized simulation scenarios.
        ...
        # LP_mod: Extract seasonal and age-stratified parameters for population generation. User-provided values override default parameter settings
        LP_season = self.pars['LP_season']          # Seasonal identifier
        LP_symp_probs = self.pars['LP_symp_probs']  # Age-specific symptom probability
        LP_trans_ORs = self.pars['LP_trans_ORs']    # Age-specific transmission odds ratios
        LP_sus_ORs = self.pars['LP_sus_ORs']        # Age-specific susceptibility odds ratios
        LP_sus_ORs[0] = self.pars['LP_sus_0_25']    # susceptibility factor for age group 0-11 years
        LP_sus_ORs[1] = self.pars['LP_sus_0_25']    # susceptibility factor for age group 12-24 years
        LP_sus_ORs[2] = self.pars['LP_sus_25_45']   # susceptibility factor for age group 25-44 years
        LP_sus_ORs[3] = self.pars['LP_sus_45_65']   # susceptibility factor for age group 45-64 years
        LP_sus_ORs[4] = self.pars['LP_sus_65']      # susceptibility factor for age group >=65 years

        # LP_mod: Create population with enhanced parameterization for influenza modeling. User-customized parameters are passed to the population generation function
        self.people = cvpop.make_people(self, reset=reset, verbose=verbose, LP_season= LP_season, LP_sus_ORs = LP_sus_ORs, LP_trans_ORs= LP_trans_ORs, LP_symp_probs=LP_symp_probs, **kwargs)
        self.people.initialize(sim_pars=self.pars) # Fully initialize the people
        self.reset_layer_pars(force=False) # Ensure that layer keys match the loaded population

        # LP_mod: Store user-defined immunological and epidemiological parameters for downstream processing
        # These parameters facilitate immunity calculations and intervention implementations
        self.people.imprt_bool = self.pars['LP_imprinting']
        self.people.season = self.pars['LP_season']
        self.people.flutype = self.pars['LP_flutype']
        self.people.start_day = self.pars['start_day']
        self.people.preHAI_bool = self.pars['LP_preHAI']
        self.people.end_tida  = self.pars['LP_end_tida']
        self.people.LP_iso_log = []    # LP_mod: Stay-at-home (self-isolation) event log for intervention tracking
    ...
    def step(self):
        ...
        for variant in range(nv):
            ...
            for lkey, layer in contacts.items():
                ...
                # LP_mod: Agents with prior infection exposure receive complete protection against reinfection
                exposed_agent = np.where(people.naive == False)   # Agents with prior infection exposure (naive=False)
                sus_imm[exposed_agent] = 1.0   # 100% immunity protection for non-naive individuals
    ...
    def compute_fit(self, *args, **kwargs):
        ...
        **Example**::
        ...
        # print ("----LP_note:sim.py: args", args)
        # print("----LP_note:sim.py: kwargs", kwargs)
        return self.fit
